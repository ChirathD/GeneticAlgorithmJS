...
function setup() {
	...
	for(let i=0; i<number_of_cities;i++){
		...
		if(Object.keys(init_city_activity).includes(i.toString())){
			city_array[i] = new City(300*Math.cos(i*Math.PI/8)+600, 300+200*Math.sin(i*Math.PI/8),init_city_activity[i])
		}
		else{
			city_array[i] = new City(300*Math.cos(i*Math.PI/8)+600, 300+200*Math.sin(i*Math.PI/8),"")
		}
